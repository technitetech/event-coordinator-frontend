...
// ─── Admin Room & Room-Type Management ───────────────────────────────────────

/** Admin: get all room types including inactive */
export async function getAllRoomTypesAdmin() {
  const pool = getPool();
  ...
  return types.map(rt => ({
    ...
    amenities_json: typeof rt.amenities_json === "string" ? JSON.parse(rt.amenities_json || "[]") : (rt.amenities_json || []),
    images_json:    typeof rt.images_json    === "string" ? JSON.parse(rt.images_json    || "[]") : (rt.images_json    || []),
    room_count: Number(rt.room_count),
  }));
}

/** Admin: get individual rooms for a room type */
export async function getRoomsForType(roomTypeId) {
  const pool = getPool();
  const [rows] = await pool.query(
    `SELECT * FROM rooms WHERE room_type_id = ? ORDER BY room_number ASC`,
    [roomTypeId]
  );
  return rows;
}
...
export async function createRoomType(data) {
  ...
  if (!slug || !data.name) return { ok: false, error: "Name and slug are required." };
  ...
  try {
    const [result] = await pool.query(
      `INSERT INTO room_types (slug, name, tagline, description, max_occupancy, size_sqm,
        base_rate_per_night, amenities_json, images_json, display_order, is_active)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [
        slug,
        sanitiseText(data.name, 120),
        sanitiseText(data.tagline, 255) || null,
        sanitiseText(data.description, 2000) || null,
        Math.max(1, parseInt(data.max_occupancy, 10) || 2),
        data.size_sqm ? Math.max(1, parseInt(data.size_sqm, 10)) : null,
        Math.max(0, parseInt(data.base_rate_per_night, 10) || 0),
    ...
    if (err.code === "ER_DUP_ENTRY") return { ok: false, error: "A room type with that slug already exists." };
    throw err;
  }
}
...
export async function updateRoomType(id, data) {
  const pool = getPool();
  const fields = {};
  if (data.name        !== undefined) fields.name               = sanitiseText(data.name, 120);
  if (data.tagline     !== undefined) fields.tagline            = sanitiseText(data.tagline, 255) || null;
  if (data.description !== undefined) fields.description        = sanitiseText(data.description, 2000) || null;
  if (data.max_occupancy !== undefined) fields.max_occupancy    = Math.max(1, parseInt(data.max_occupancy, 10) || 2);
  if (data.size_sqm    !== undefined) fields.size_sqm           = data.size_sqm ? Math.max(1, parseInt(data.size_sqm, 10)) : null;
  if (data.base_rate_per_night !== undefined) fields.base_rate_per_night = Math.max(0, parseInt(data.base_rate_per_night, 10) || 0);
  if (data.display_order !== undefined) fields.display_order    = parseInt(data.display_order, 10) || 0;
  if (data.is_active   !== undefined) fields.is_active          = data.is_active ? 1 : 0;
  if (data.amenities   !== undefined) fields.amenities_json     = JSON.stringify(Array.isArray(data.amenities) ? data.amenities.filter(Boolean) : []);
  if (data.images      !== undefined) fields.images_json        = JSON.stringify(Array.isArray(data.images)    ? data.images.filter(Boolean)    : []);

  if (Object.keys(fields).length === 0) return { ok: false, error: "No fields to update." };
  const sets = Object.keys(fields).map(k => `${k} = ?`).join(", ");
  await pool.query(`UPDATE room_types SET ${sets} WHERE id = ?`, [...Object.values(fields), id]);
  return { ok: true };
}

/** Admin: delete a room type (cascades to rooms via FK) */
export async function deleteRoomType(id) {
  const pool = getPool();
  const [bookings] = await pool.query(
    `SELECT COUNT(*) AS cnt FROM stay_bookings sb
     JOIN rooms r ON r.id = sb.room_id
     WHERE r.room_type_id = ? AND sb.status NOT IN ('cancelled','no_show','checked_out')`,
    [id]
  );
  if (Number(bookings[0].cnt) > 0) {
    return { ok: false, error: "Cannot delete: there are active bookings for rooms of this type." };
  }
  await pool.query(`DELETE FROM room_types WHERE id = ?`, [id]);
  return { ok: true };
}

/** Admin: add a physical room to a room type */
export async function createRoom(data) {
  ...
  try {
    const [result] = await pool.query(
      ...
      [
        data.room_type_id,
        String(data.room_number).trim().toUpperCase(),
        parseInt(data.floor, 10) || 1,
        data.status || "available",
        sanitiseText(data.notes, 500) || null,
      ]
    );
    return { ok: true, id: result.insertId };
  } catch (err) {
    if (err.code === "ER_DUP_ENTRY") return { ok: false, error: "Room number already exists." };
    throw err;
  }
}
...
export async function updateRoom(id, data) {
  ...
  const status = valid.includes(data.status) ? data.status : undefined;
  const sets = [];
  const vals = [];
  if (data.room_number !== undefined) { sets.push("room_number = ?"); vals.push(String(data.room_number).trim().toUpperCase()); }
  if (data.floor       !== undefined) { sets.push("floor = ?");       vals.push(parseInt(data.floor, 10) || 1); }
  if (status           !== undefined) { sets.push("status = ?");      vals.push(status); }
  if (data.notes       !== undefined) { sets.push("notes = ?");       vals.push(sanitiseText(data.notes, 500) || null); }
...
export async function deleteRoom(id) {
  ...
  const [active] = await pool.query(
    `SELECT COUNT(*) AS cnt FROM stay_bookings
     WHERE room_id = ? AND status NOT IN ('cancelled','no_show','checked_out')`, [id]
  );
  if (Number(active[0].cnt) > 0) {
    return { ok: false, error: "Cannot delete: this room has active bookings." };
  }
  await pool.query(`DELETE FROM rooms WHERE id = ?`, [id]);
  return { ok: true };
}

// ─── Admin stay-booking management ───────────────────────────────────────────

/** Admin: cancel a stay booking regardless of notice period */
export async function adminCancelStayBooking(bookingId, reason) {
  const pool = getPool();
  const [rows] = await pool.query(`SELECT status FROM stay_bookings WHERE id = ? LIMIT 1`, [bookingId]);
  if (!rows[0]) return { ok: false, error: "Booking not found." };
  const check = assertTransition("stay_bookings", rows[0].status, "cancelled");
  if (!check.ok) return { ok: false, error: check.error };
  await pool.query(
    `UPDATE stay_bookings SET status = 'cancelled', cancelled_at = NOW(), cancellation_reason = ? WHERE id = ?`,
    [sanitiseText(reason, 500) || "Admin cancellation", bookingId]
  );
  return { ok: true };
}
