...
export async function cancelDiningReservation(reservationId, userId, reason) {
  ...
  const slotAt = new Date(res.reservation_date);
  ...
  await pool.query(
    `UPDATE dining_reservations SET status = 'cancelled', special_requests = COALESCE(special_requests, ?) WHERE id = ?`,
    [sanitiseText(reason, 300), reservationId]
  );
  return { ok: true };
}
...
/** Get all menu items (including unavailable) with their category name */
export async function getMenuItemsAdmin() {
  const pool = getPool();
  const [rows] = await pool.query(
    `SELECT rmi.*, rc.name AS category_name
     FROM restaurant_menu_items rmi
     JOIN restaurant_categories rc ON rc.id = rmi.category_id
     ORDER BY rc.display_order ASC, rmi.display_order ASC, rmi.name ASC`
  );
  return rows;
}
...
const ALLOWED_ITEM_FIELDS = [
  ...
  "is_spicy", "is_signature", "image_url", "allergens", "display_order",
];

function buildItemPayload(data) {
  const payload = {};
  for (const field of ALLOWED_ITEM_FIELDS) {
    if (data[field] !== undefined) payload[field] = data[field];
  }
  if (payload.name !== undefined) payload.name = sanitiseText(payload.name, 120);
  if (payload.description !== undefined) payload.description = sanitiseText(payload.description, 500);
  if (payload.allergens !== undefined) payload.allergens = sanitiseText(payload.allergens, 255);
  if (payload.price !== undefined) payload.price = Math.max(0, Math.round(Number(payload.price)));
  for (const flag of ["is_available","is_vegetarian","is_vegan","is_gluten_free","is_spicy","is_signature"]) {
    if (payload[flag] !== undefined) payload[flag] = payload[flag] ? 1 : 0;
  }
  return payload;
}
...
export async function createMenuItem(data) {
  ...
  if (!payload.name) return { ok: false, error: "Item name is required." };
  if (!payload.category_id) return { ok: false, error: "Category is required." };
  if (payload.price == null) return { ok: false, error: "Price is required." };
  payload.is_available = payload.is_available ?? 1;

  const cols = Object.keys(payload).join(", ");
  const placeholders = Object.keys(payload).map(() => "?").join(", ");
  const [result] = await pool.query(
    `INSERT INTO restaurant_menu_items (${cols}) VALUES (${placeholders})`,
...
export async function updateMenuItem(id, data) {
  const pool = getPool();
  const payload = buildItemPayload(data);
  if (Object.keys(payload).length === 0) return { ok: false, error: "No fields to update." };

  const sets = Object.keys(payload).map((k) => `${k} = ?`).join(", ");
  await pool.query(
    `UPDATE restaurant_menu_items SET ${sets} WHERE id = ?`,
...
export async function deleteMenuItem(id) {
  const pool = getPool();
  await pool.query(`DELETE FROM restaurant_menu_items WHERE id = ?`, [id]);
  return { ok: true };
}
...
export async function toggleMenuItemAvailability(id) {
  const pool = getPool();
  await pool.query(
    `UPDATE restaurant_menu_items SET is_available = NOT is_available WHERE id = ?`, [id]
  );
  return { ok: true };
}
