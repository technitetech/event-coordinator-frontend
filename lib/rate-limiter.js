 ...
 *
 * Resets on server restart — good enough for a single-instance deployment.
 * For multi-instance / serverless, swap the Map for a Redis INCR+EXPIRE call.
 *
 * Usage:
 *   const result = checkRateLimit(`login:${ip}`, 5, 60_000); // 5 req / 60 s
 *   if (!result.ok) return { ok: false, error: `Too many attempts. Retry in ${result.retryAfter}s.` };
 */
